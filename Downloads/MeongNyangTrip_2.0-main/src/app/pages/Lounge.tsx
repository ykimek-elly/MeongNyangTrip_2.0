...
const handleCreatePost = (
  ...
  const finalImg = typeof imgSource === 'number' 
    ? SAMPLE_IMAGES[imgSource % SAMPLE_IMAGES.length] 
    : imgSource[0]; // myImages 배열의 첫 번째 사진 사용

  addPost({
    user: "나",
    userImg: "https://images.unsplash.com/photo-1535930749574-1399327ce78f?w=100&q=80",
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm pointer-events-auto">
            ...
            >
              <button onClick={() => setIsMenuOpen(false)} className="absolute top-5 right-5 text-gray-400">
                <X size={24} />
              </button>

              <h3 className="text-center font-bold text-gray-900 mb-8 text-lg">어떤 글을 남길까요?</h3>
