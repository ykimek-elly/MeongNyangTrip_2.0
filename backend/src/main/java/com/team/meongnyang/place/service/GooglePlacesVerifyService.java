...
public class GooglePlacesVerifyService {
    ...
    private static final String FIELD_MASK =
            "places.displayName,places.allowsDogs,places.businessStatus,places.location";
    ...
    public EnrichResult enrich(String name, String address) {
        if (apiKey == null || apiKey.isBlank()) {
            ...
            return new EnrichResult(false, false);
        }
        ...
        try {
            ...
            if (response == null) return new EnrichResult(false, false);

            List<Map<String, Object>> places =
                    (List<Map<String, Object>>) response.get("places");
            if (places == null || places.isEmpty()) return new EnrichResult(false, false);
            ...
            log.debug("[Google] {} → allowsDogs={}, status={}", name, petOk, status);
            return new EnrichResult(petOk, closed);
