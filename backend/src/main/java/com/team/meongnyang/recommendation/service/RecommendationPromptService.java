...
public class RecommendationPromptService {
  ...
  public String buildRecommendationPrompt(RecommendationEvidenceContext context) {
    ...
    return """
          ...
          - 추천 결과는 이미 필터링/스코어링을 통과한 상태이므로 설명은 판정이 아닌 근거 정리 역할에 집중한다.
          - 억지로 항목을 나열하지 말고 하나의 흐름으로 읽히게 쓴다.
          - 사람이 직접 설명하듯 자연스럽고 읽기 쉽게 쓴다.

          [의미 기반 선호 매칭 규칙]
          - 선호 장소 단어가 완전히 동일하지 않아도 의미가 유사하면 일치로 해석한다.
          - 공원, 녹지, 산책로, 자연 공간은 "자연/숲" 계열 선호와 긍정적으로 연결한다.
          - 운동장, 광장, 넓은 야외 공간은 "운동장/활동 공간" 계열 선호와 긍정적으로 연결한다.
          - category, tags, overview, description, 시설 정보에서 발견되는 의미 단서를 함께 사용해 선호와 연결한다.
          - 의미상 맞는 선호 항목이 확인되면 해당 연결을 설명의 핵심 근거로 우선 사용한다.
          - 추천된 장소에 대해 "완전히 일치하지 않는다", "선호와 거리가 있다", "상쇄된다" 같은 약화 문장은 사용하지 않는다.
