...
> 작성일: 2026-03-14 | 담당: 팀원D (FE & Interface + 백엔드 통합)
> 이전 세션(03-13) 내용 포함 누적 정리
...
| 우선순위 | 작업 | 선행 조건 |
|:---:|---|---|
| ★★★ | `VITE_USE_MOCK=false` 전환 → 장소 API 실연동 테스트 | 백엔드 서버 실행 |
| ★★★ | PetController + PetService 구현 (`/api/v1/pets` CRUD) | Pet Entity/Repository ✅ |
| ★★★ | DB Entity 생성 — REVIEW, WISHLIST, FEED, FEED_LIKE, COMMENT, TALK, TALK_COMMENT | — |

### 팀원 작업 완료 후 연동

| 우선순위 | 작업 | 선행 조건 |
|:---:|---|---|
| ★★☆ | JWT 인증 연동 (로그인/회원가입) | 팀원A SecurityConfig 완료 후 |
